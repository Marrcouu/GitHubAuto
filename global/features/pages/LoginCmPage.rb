...
class LoginCmPage
  ...
  class << self
    ...
    #Funcion para validar los elementos de inicio de sesion en partner claro música.
    def validate_page
      find(:id,'username', wait: 10)
      find(:id, 'password', wait:10)
      #all(:css, "li.tab_selector", minimum:4, wait:10)[3].has_css?(".active")
    end
    ...
    #Función que llena los campos para inicio de sesión en partner claro música,
    ...
    def fill_cm_fields
      ...
      find(:css,'input[name="username"]', wait: 20).native.send_keys credentials[:email] if credentials[:email] != 'random_new' and credentials[:email] != 'random_old' and $brow == 'ie'
      find(:css,'input[name="username"]', wait: 20).native.send_keys email_random("new") if credentials[:email] == 'random_new' and $brow == 'ie'
      find(:css,'input[name="username"]', wait: 20).native.send_keys email_random("old") if credentials[:email] == 'random_old' and $brow == 'ie'
      find(:id, 'password', wait:20).native.send_keys credentials[:password] if credentials[:password]
      find(:id, 'password', wait:20).native.send_keys credentials[:password] if credentials[:password] and $brow == 'ie'
    end
    ...
    def error_message(error)
      ...
      when :'empty fields'
        all(:css, 'span.error-validator.ng-star-inserted', visible:true, maximum:2, wait:30)
      when :'invalid email', :'user erased',:'user claro video', :'user telmex',:'check your credit card',
          :'another number telmex', :'invalid number telmex', :'short number', :'wrong password'
        all(:css, 'div.error-backend.ng-star-inserted', wait: 30)
      when :'not telcel', :'number invalid', :'missing number', :'check your credit card',
          :'another number', :'missing password'
        all(:id, 'error-message', wait:60)
